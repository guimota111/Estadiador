...
import { prefix, axillaryPN, stagingLine, PN_OPTIONS } from './_shared.js';
...
export default {
  ...
  compute(v) {
    ...
    let mSuffix = (v.ptMode === 'size' && v.multi === 'yes') ? '(m)' : '';
    const pTtoken = Tcat ? `${pre}${Tcat}${mSuffix}` : `${pre}TX`;

    /* ---------- pN ---------- */
    let Ncat = null, nDesc = '';
    if (v.nodeInput === 'none') {
      Ncat = null; nDesc = 'não atribuído (sem linfonodos)';
    } else if (v.nodeInput === 'manual') {
      Ncat = v.pnManual;
      nDesc = (PN_OPTIONS.find(o => o.value === Ncat) || {}).label || '';
    } else {
      const r = axillaryPN(v.nMacro, v.nMicro, v.nITC);
      Ncat = r.cat; nDesc = r.desc;
    }
