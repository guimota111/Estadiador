...
export default {
  ...
  compute(v) {
    const warnings = [];
    const pos = v.nPositive, exam = v.nExamined;

    // --- Categoria N ---
    let N = null, nDesc = '';
    if (pos == null) { N = null; nDesc = 'não avaliável'; }
    else if (pos === 0) { N = 'N0'; nDesc = 'sem metástase regional'; }
    else if (pos <= 2)  { N = 'N1'; nDesc = `${pos} linfonodos positivos`; }
    else if (pos <= 6)  { N = 'N2'; nDesc = `${pos} linfonodos positivos`; }
    else if (pos <= 15) { N = 'N3a'; nDesc = `${pos} linfonodos positivos`; }
    else                { N = 'N3b'; nDesc = `${pos} linfonodos positivos`; }
